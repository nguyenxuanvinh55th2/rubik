...
var Calendar = (function (_Component) {
  ...
  _createClass(Calendar, [{
    ...
    key: 'changeYear',
    value: function changeYear(number) {
      var _props3 = this.props;
      var link = _props3.link;
      var linkCBYear = _props3.linkCBYear;

      if(link && linkCBYear){
          return linkCBYear(number);
      }
      var current = this.state.shownDate.year();
      var newYear = this.state.shownDate.clone().year(number);
    ...
    value: function renderMonthAndYear(classes) {
      ...
      var arrayYear = [];
      for(let i=-20;i<11;i++){
          arrayYear.push(shownDate.clone().add(i, 'year').format('YYYY'));
      }
      var selectedYearOptions = arrayYear.map((option, idx) =>_react2['default'].createElement( 'option', { key: idx, value: option },  option ));
      return _react2['default'].createElement(
        ...
        _react2['default'].createElement(
          ...
          _react2['default'].createElement(
            'select',
            { className: classes.year, value: year, onChange: ({target})=>this.changeYear(target.value) },
            selectedYearOptions
          )
        ),
        showMonthArrow ? _react2['default'].createElement(
